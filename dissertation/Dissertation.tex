...
The project also provides experience relevant to future work in artificial
intelligence and software development. It involves practical work in language
model fine-tuning, dataset construction, automated evaluation and symbolic
reasoning, together with the design of a reproducible experimental pipeline.
These skills are applicable to future work involving machine learning,
generative AI and the development and evaluation of reliable AI systems.
...
reasoning. This approach also produced strong results when using an open-source
model smaller than the largest proprietary models evaluated in the study.
...
understanding with symbolic reasoning. Autoformalization work has shown
that language models can translate informal statements into formal languages,
while Logic-LM and LINC demonstrate the value of delegating inference to
deterministic symbolic solvers. RuleTaker and PARARULE-Plus further show
how controlled natural-language datasets can be used to investigate logical
reasoning systematically. Recent Prolog-based work also demonstrates
growing interest in using executable logic programs as an interface between
language models and formal reasoning.
...
At the same time, successful execution cannot by itself establish that a
generated program faithfully represents the natural-language input. A program
may be syntactically valid and execute successfully while using an incorrect
predicate or reversing the arguments of a relation. Symbolic execution can
therefore verify properties of the representation it receives, but it cannot
by itself verify that the representation is a semantically faithful
formalization of the original statement.
...
The original NL--Prolog dataset can be generated from the project root using:
...
NL--Prolog dataset, implementation of a reproducible training and evaluation
...
language-model fine-tuning, quantisation, symbolic reasoning and automated
evaluation. It also demonstrated the importance of careful experimental design
and of evaluating generated formal representations using multiple measures of
correctness. While the resulting system is limited to a controlled domain, I
believe the completed implementation and experiments successfully address the
main aims of the project and provide a useful basis for further work.

The project also taught me the importance of establishing reproducible
datasets and evaluation procedures early, maintaining clear intermediate
milestones, and allowing sufficient time for error analysis and experimental
refinement.
