...
validation of an NL--Prolog dataset, the establishment of an untuned baseline,
...
RuleTaker demonstrates that transformer models can learn patterns corresponding
to deductive reasoning. However, this distinction between learning the
behaviour of a reasoning process and executing a formal reasoning procedure
remains important. A neural model may produce the correct answer without
providing a representation that can independently be checked by a deterministic
solver. This motivates approaches in which language understanding and formal
inference are separated.
...
proving. This demonstrated that language models could serve as an interface
between informal human language and representations suitable for formal
verification.
...
ProofWriter and FOLIO, and produced substantial improvements over both standard
prompting and chain-of-thought prompting. This framework also included a
refinement stage that used error messages from the symbolic solver to revise
...
theorem prover. Their experiments on ProofWriter and FOLIO found that this
separation could outperform direct chain-of-thought reasoning. This approach
also produced strong results when using an open-source model smaller than the
largest proprietary models evaluated in the study.
...
For this reason, a dedicated NL--Prolog dataset was constructed rather than
...
therefore focus on whether a small model can learn NL--Prolog translation
...
Previous research provides strong evidence for combining neural language
understanding with symbolic reasoning. Autoformalization work has shown that
language models can translate informal statements into formal languages, while
Logic-LM and LINC demonstrate the value of delegating inference to
deterministic symbolic solvers. RuleTaker and PARARULE-Plus further show how
controlled natural-language datasets can be used to investigate logical
reasoning systematically. Recent Prolog-based work also demonstrates growing
interest in using executable logic programs as an interface between language
models and formal reasoning.

The present project focuses on a narrower question within this area. This
project does not test whether a large general-purpose model can be prompted to
produce symbolic representations; instead, it investigates whether a
1.5-billion-parameter model can be explicitly trained to perform NL--Prolog
translation. It also evaluates the generated output at several levels,
separating exact textual agreement from syntax validity, executability and
reasoning correctness. This provides a practical investigation of whether
relatively small language models can learn a reliable interface between
natural language and executable symbolic reasoning.
...
ProofWriter and PARARULE-Plus provide large collections of controlled
...
using a reasoning engine. ProofWriter extends this style of dataset with rule
...
\begin{lstlisting}
Natural language: Mary is a doctor.
Prolog: doctor(mary).
\end{lstlisting}
...
\begin{lstlisting}
Natural language: Every doctor is a professional.
Prolog: professional(X) :- doctor(X).
\end{lstlisting}
...
to datasets such as ProofWriter and PARARULE-Plus, where reasoning problems are
...
The test set was kept fixed throughout the baseline, initial fine-tuning and
refinement experiments. Maintaining the same test data made direct comparisons
between model versions more reliable.

\subsection{Final Dataset}

The resulting dataset contains 2,000 validated NL--Prolog pairs covering facts,
...
QLoRA was used to adapt Qwen2.5-1.5B-Instruct to the NL--Prolog translation
task. The implementation combined 4-bit loading of the base model with trainable
LoRA adapter layers.

The base model was loaded in 4-bit quantised form. Its original parameters were
frozen during training. Trainable low-rank adapter parameters were inserted into
selected model layers, and only these adapter parameters were updated. The
resulting training process required substantially less GPU memory than updating
all model parameters.

The first fine-tuning run used the 1,597 examples in the training split. The 204
validation examples were kept separate and were not included in the training
data. Training was performed for three epochs with a per-device batch size of
two. The random seed was set to \texttt{4242564}.

Training was carried out using the TRL supervised fine-tuning pipeline. The
model used float16 computation where supported, while the base model remained
loaded using 4-bit quantisation. The trained adapter was saved separately from
the base Qwen model. During inference, the adapter was loaded together with the
original base model.
...
original NL--Prolog training set and evaluated under the same conditions. The
...
Qwen2.5-1.5B-Instruct model could perform NL--Prolog translation without
...
code-like text, but this did not translate into reliable NL--Prolog
formalization.
...
Reasoning correctness also increased substantially, from 14.63\% to 87.80\%.
...
specialised NL--Prolog generation behaviour using consumer hardware.
...
Reasoning Correct & 87.80\%  & \textbf{100.00\%} \\
\bottomrule
\end{tabular}
\end{table}
...
Most notably, reasoning correctness increased to 41 out of 41 examples. All
Level 4 queries therefore produced their expected results under the final
model.
...
The largest improvement occurred between the baseline and first fine-tuned
model. This indicates that supervised task-specific adaptation was responsible
for most of the performance gain. Refinement produced a smaller but still
measurable additional improvement, particularly for simple and multi-condition
rules.
...
refinement stage produced a further increase in exact-match and reasoning
accuracy while maintaining 100\% syntax validity and executability.
...
final model, shows that Qwen2.5-1.5B can acquire strong NL--Prolog translation
...
useful semantic information beyond execution-unaware comparison. Candidate
...
Reasoning accuracy showed one of the largest improvements, increasing from
14.63\% for the baseline to 87.80\% for the first fine-tuned model and 100\%
for the refined model. This provides evidence that the generated programs were
not merely syntactically well formed. For the 41 reasoning examples in the
final test set, every program supported the expected query result.
...
87.94\%, reduced non-exact outputs from 37 to 24 and increased reasoning
correctness to 100\%. The improvement indicates that error analysis can be used
to guide additional training more efficiently than simply generating more
examples without considering model behaviour.
...
executable. Reasoning correctness was 14.63\%. These results demonstrated that
general instruction-following ability alone was insufficient for reliable
natural-language-to-Prolog translation.
...
syntactically valid and executable. Reasoning accuracy increased to 87.80\%.
...
model can be fine-tuned to perform NL--Prolog autoformalization with high
reliability on a controlled domain, and parameter-efficient training makes
this possible using consumer hardware. The project also demonstrates the
usefulness of Prolog execution as an evaluation mechanism. Exact match alone
does not fully describe model performance: an output can differ from the
reference while remaining executable, while an executable program may still
encode an incorrect semantic relationship. Combining exact match with syntax,
execution and reasoning evaluation therefore gives a more informative picture
of autoformalization quality.

There are several limitations to these conclusions. The dataset was
synthetically generated from controlled templates and consequently does not
represent the full linguistic variation of naturally written reasoning
problems. The test set was also relatively small. In addition, the refinement
examples were designed after inspecting weaknesses observed on the test set,
meaning that the final 87.94\% result should not be interpreted as a fully
independent estimate of generalisation. The refinement experiment demonstrates
improvement on identified weaknesses, but a completely untouched final test
set would be required for stronger evaluation.
...
Overall, the project demonstrates that relatively small language models can
learn a useful interface between natural language and symbolic logic. Although
the resulting model is not a general-purpose autoformalizer, the substantial
improvement from the untuned baseline and the reliability of executable output
show that combining task-specific language-model training with symbolic
validation is a practical approach to producing more checkable forms of
machine reasoning.
